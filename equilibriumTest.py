...
acceptance = defineProbability(beta)        # acceptance is the probability function for change acceptance
...
for i in range(iterations):     # Computes the changes in the system and shows the simulation
    list(map(sortModifyIncrement1, material))
    list(map(boundryControl, material))
    Emod = systemEnergy(material, True)
    if (acceptance(E, Emod)):
        E = Emod
        list(map(modifyPos, material))
        X = list(map(getX, material))
        Y = list(map(getY, material))
    Energies.append(E)
    system.set_xdata(X)
