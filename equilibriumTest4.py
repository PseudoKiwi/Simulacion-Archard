from numpy import array, pi, sin, cos, sqrt, inf, zeros
...
def systemEnergy(pos, inc, withIncrements = False):  # Computes the total interaction energy of the system
...
def sortModifyIncrement1(inc):   # Sorts and modify de increment of every particle
    for i in range(nParticles):
        angle = 2 * pi * random()
        dr = r01/5 * random()
...
def modifyPos(pos, inc):    # Modifies the position of every particle
    for i in range(nParticles):
        pos[0][i] += inc[0][i]
        pos[1][i] += inc[1][i]


def boundryControl(pos, inc):    # Controls the particles do not get out of the boundries
    for i in range(nParticles):
...
E = systemEnergy(position, increments)
energies[0] = E
...
for i in range(iterations):     # Computes the changes in the system and shows the simulation
    sortModifyIncrement1(increments)
    boundryControl(position, increments)
    Emod = systemEnergy(position, increments, True)
    dE = Emod - E
    if (acceptance(dE)):
        E += dE
        modifyPos(position, increments)
        X = position[0]  # Every x position
        Y = position[1]  # Every y position
    energies[0][i+1] = E
    system.set_xdata(X)
